 ...
 (function($) {
	...
	Calendar.prototype = {
		...
		_initializeEvents: function(opt) {
			...
			if(opt.doubleClickDay) { this.element.bind('doubleClickDay', opt.doubleClickDay); }
		...
		_initializeDatasourceColors: function() {
			for(var i in this.options.dataSource) {
				if(this.options.dataSource[i].color == null) {
					if(this.options.dataSource[i].wtype == null) {
						this.options.dataSource[i].color = colors[i % colors.length];
					}
					else {
						if(this.options.dataSource[i].wtype == 'vacation') {    //假期
							this.options.dataSource[i].color = colors[i % colors.length];
						}
						else if(this.options.dataSource[i].wtype == 'work') {   //工作
							this.options.dataSource[i].color = colors2[i % colors2.length];
						}
					}
		...
		_applyEvents: function () {
			...
			cells.dblclick(function(e){
				e.stopPropagation();
				var date = _this._getDate($(this));
				_this._triggerEvent('doubleClickDay', {
					element: $(this),
					which: e.which,
					date: date,
					events: _this.getEvents(date)
	...
	$.fn.doubleClickDay = function(fct) { $(this).bind('doubleClickDay', fct); }
	...
	var colors = $.fn.calendar.colors = ['#2C8FC9'];		//假期
	var colors2 = $.fn.calendar.colors2 = ['#FF4A32'];  	//工作
